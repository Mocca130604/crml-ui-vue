<template>
  <div class="storybook-root font-mono">
    <!-- Storybook Header Banner -->
    <header class="storybook-top-header">
      <div class="header-left">
        <CrmlStickerBadge variant="pink" tilt="none" star>STORYBOOK v5.5</CrmlStickerBadge>
        <h1 class="storybook-title font-heading">
          CRML - UI <span class="title-highlight">COMPONENT EXPLORER & STORYBOOK</span>
        </h1>
      </div>
    ...
    <!-- Main Storybook Workspace Grid -->
    <div class="storybook-workspace">
      <!-- Left Sidebar: Component Catalog -->
      <aside class="storybook-sidebar">
        <div class="catalog-search">
          <CrmlInput v-model="searchQuery" :placeholder="`Filter ${totalComponentCount} components...`" />
        </div>

        <nav class="catalog-nav">
          <template v-for="cat in filteredCatalog" :key="cat.name">
            <div class="catalog-category-header font-heading">
            ...
            <button
              ...
              :key="comp.id"
              class="catalog-item-btn font-mono"
              :class="{ active: selectedCompId === comp.id }"
              @click="selectedCompId = comp.id"
            >
              <span class="comp-icon">{{ comp.icon || '📦' }}</span>
              <span class="comp-name">{{ comp.name }}</span>
              <span v-if="comp.badge" class="comp-badge-chip">{{ comp.badge }}</span>
            </button>
          </template>
        </nav>
      </aside>

      <!-- Right Main Panel: Component Inspector & Sandbox -->
      <main class="storybook-main">
        <!-- Component Title & Description Header -->
        <div class="inspector-header-card">
          <div class="header-title-row">
            <div>
              <span class="comp-vol-tag font-mono">// {{ activeComp.category }}</span>
              <h2 class="active-comp-name font-heading">{{ activeComp.name }}</h2>
            </div>
            <div class="header-badges-cluster">
              <CrmlBadge variant="success" class="font-mono">{{ activeComp.type }}</CrmlBadge>
              <CrmlBadge variant="cyan" class="font-mono">{{ activeComp.category }}</CrmlBadge>
              <CrmlBadge variant="obsidian" class="font-mono">0% BLUR</CrmlBadge>
              <CrmlBadge variant="pink" class="font-mono">TACTILE REBOUND</CrmlBadge>
            </div>
          </div>
          <p class="active-comp-desc">{{ activeComp.desc }}</p>

          <div v-if="activeComp.designNotes" class="design-guidelines-box font-mono">
            <div class="guideline-header">
              <span>💡 DESIGN SYSTEM SPECIFICATION & MODULARITY NOTES</span>
            </div>
            <p class="guideline-content">{{ activeComp.designNotes }}</p>
          </div>
        </div>

        <!-- Interactive Sandbox & Controls Row -->
        <div class="sandbox-split-row">
          <!-- Live Preview Box -->
          <div class="live-preview-box">
            <div class="box-bar font-mono">
              <span>LIVE PREVIEW CANVAS</span>
              <span class="pulse-dot">● ONLINE</span>
            </div>

            <div class="canvas-display">
              <!-- Render Component Dynamically Based on selectedCompId -->
              <template v-if="selectedCompId === 'CrmlBouncyButton'">
                <CrmlBouncyButton
                  :variant="activeVariant"
                  :size="activeSize"
                  :disabled="activeDisabled"
                  :icon-left="btnIconLeft"
                  :icon-right="btnIconRight"
                >
                  {{ customLabel || 'Tactile Button' }}
                </CrmlBouncyButton>
              </template>

              <template v-else-if="selectedCompId === 'CrmlButton'">
                <CrmlButton
                  :variant="activeVariant === 'lime' ? 'primary' : (activeVariant === 'pink' ? 'danger' : (activeVariant === 'cyan' ? 'secondary' : 'outline'))"
                  :size="activeSize"
                  :disabled="activeDisabled"
                  :icon-left="btnIconLeft"
                  :icon-right="btnIconRight"
                >
                  {{ customLabel || 'Neubrutal Button' }}
                </CrmlButton>
              </template>

              <template v-else-if="selectedCompId === 'CrmlBadge'">
                <CrmlBadge :variant="activeVariant === 'lime' ? 'success' : (activeVariant === 'pink' ? 'danger' : (activeVariant === 'cyan' ? 'primary' : 'warning'))" :size="activeSize === 'lg' ? 'md' : activeSize">
                  {{ customLabel || 'SYSTEM STATUS' }}
                </CrmlBadge>
              </template>

              <template v-else-if="selectedCompId === 'CrmlStickerBadge'">
                <CrmlStickerBadge :variant="activeVariant" :star="activeStar" :tilt="activeTilt">
                  {{ customLabel || '🔥 HOT DROP' }}
                </CrmlStickerBadge>
              </template>

              <template v-else-if="selectedCompId === 'CrmlStampBadge'">
                <CrmlStampBadge :variant="activeVariant === 'pink' ? 'danger' : (activeVariant === 'yellow' ? 'warning' : (activeVariant === 'cyan' ? 'info' : 'success'))">
                  {{ customLabel || 'VERIFIED PILOT' }}
                </CrmlStampBadge>
              </template>

              <template v-else-if="selectedCompId === 'CrmlMarquee'">
                <div style="width: 100%;">
                  <CrmlMarquee :text="customLabel || '⚡ CRML MARQUEE TICKER BANNER ⚡'" :variant="activeVariant" :speed="marqueeSpeed" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlBlobAvatar'">
                <CrmlBlobAvatar :text="customLabel ? customLabel.charAt(0) : 'K'" :variant="activeVariant" :size="activeSize" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlSpeechBubble'">
                <CrmlSpeechBubble :variant="activeVariant === 'cyan' ? 'lime' : activeVariant">
                  {{ customLabel || 'Synthesizing 56 primitives with zero ambient blur!' }}
                </CrmlSpeechBubble>
              </template>

              <template v-else-if="selectedCompId === 'CrmlPixelIconWrapper'">
              ...
              <template v-else-if="selectedCompId === 'CrmlWavyDivider'">
                <div style="width: 100%;">
                  <CrmlWavyDivider :style-type="dividerStyle" :color="activeVariant === 'pink' ? '#FF007F' : (activeVariant === 'cyan' ? '#00F0FF' : (activeVariant === 'yellow' ? '#FFD600' : '#CCFF00'))" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlDatePicker'">
                <CrmlDatePicker
                  v-model="dateVal"
                  :mode="dateMode"
                  :variant="activeVariant"
                  :label="customLabel || 'TACTILE DATE PICKER'"
                />
              </template>

              <template v-else-if="selectedCompId === 'CrmlTimePicker'">
                <CrmlTimePicker v-model="timeVal" :variant="activeVariant" :format="timeFormatVal" :label="customLabel || 'NEUBRUTAL TIME PICKER'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlNumberInput'">
                <CrmlNumberInput v-model="numVal" :variant="activeVariant" :disabled="activeDisabled" :label="customLabel || 'QUANTITY SPINNER'" :unit="unitBadgeText" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlInput'">
                <div style="width: 100%; max-width: 320px;">
                  <CrmlInput v-model="inputVal" :variant="activeVariant" :disabled="activeDisabled" :label="customLabel || 'USERNAME HANDLE'" placeholder="Type here..." />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlTextarea'">
                <div style="width: 100%; max-width: 380px;">
                  <CrmlTextarea v-model="textareaVal" :variant="activeVariant" :disabled="activeDisabled" :label="customLabel || 'CRATE DESCRIPTION'" placeholder="Type multi-line content..." :max-length="120" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlRichTextEditor'">
                <div style="width: 100%;">
                  <CrmlRichTextEditor v-model="richVal" :title="customLabel || 'STORYBOOK_DOC.TXT'" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlColorPicker'">
                <CrmlColorPicker v-model="colorVal" :label="customLabel || 'SURFACE COLOR'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlDualRangeSlider'">
                <div style="width: 100%; max-width: 360px;">
                  <CrmlDualRangeSlider
                    v-model:min="rangeMin"
                    v-model:max="rangeMax"
                    :min-limit="0"
                    :max-limit="rangeCompact ? 10000000 : 1000"
                    :variant="activeVariant"
                    :label="customLabel || 'PRICE RANGE FILTER'"
                    :prefix="rangePrefix"
                    :suffix="rangeSuffix"
                    :compact="rangeCompact"
                    :show-limits="showRangeLimits"
                  />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlCheckbox'">
                <CrmlCheckbox v-model="checkboxVal" :variant="activeVariant" :disabled="activeDisabled" :label="customLabel || 'ENABLE REBOUND PHYSICS'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlNeonSwitch'">
                <CrmlNeonSwitch v-model="switchVal" :variant="activeVariant" :disabled="activeDisabled" :label="customLabel || 'NEON OVERDRIVE MODE'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlSegmentedControl'">
                <CrmlSegmentedControl v-model="segmentedVal" :options="variantOptions" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlOtpInput'">
                <CrmlOtpInput v-model="otpVal" :variant="activeVariant" :disabled="activeDisabled" :length="6" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlFaceIdToggle'">
                <CrmlFaceIdToggle :variant="activeVariant" :label="customLabel || 'BIOMETRIC LOCK'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlAiPromptInput'">
                <div style="width: 100%;">
                  <CrmlAiPromptInput :variant="activeVariant" :disabled="activeDisabled" :placeholder="customLabel || 'Generate a neubrutalist UI layout...'" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlFileUpload'">
                <div style="width: 100%;">
                  <CrmlFileUpload
                    :title="customLabel || 'DRAG & DROP ASSETS HERE'"
                    description="SUPPORTED: .FIG, .JSON, .PNG (MAX 50MB)"
                    progress-file-name="zine_master_v3.fig"
              ...
              <template v-else-if="selectedCompId === 'CrmlSpeedometerDial'">
                <CrmlSpeedometerDial :value="progressVal" :variant="activeVariant" :title="customLabel || 'CPU LOAD TELEMETRY'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlRadarTelemetry'">
                <CrmlRadarTelemetry :variant="activeVariant" :title="customLabel || 'RADAR TELEMETRY'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlSoundboard'">
                <CrmlSoundboard :variant="activeVariant" :title="customLabel || 'MPC SOUNDBOARD'" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlStatusMatrix'">
                <div style="width: 100%;">
                  <CrmlStatusMatrix :title="customLabel || 'SYSTEM NODE STATUS MATRIX'" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlTable'">
                <div style="width: 100%;">
                  <CrmlTable :columns="tableColumns" :data="tableData" :accent-color="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlPriceTagWidget'">
                <CrmlPriceTagWidget
                  :title="customLabel || 'MONTHLY DROP REVENUE'"
                  price="128.4K"
                  badge-text="▲ +24.8%"
                  :stock-current="progressVal"
                  :stock-total="100"
                  button-text="CHECK TELEMETRY ⚡"
                  :variant="activeVariant"
                />
              </template>

              <template v-else-if="selectedCompId === 'CrmlCodeBlock'">
                <div style="width: 100%;">
                  <CrmlCodeBlock :code="codeSnippetSample" :title="customLabel || 'install_crml_ui.sh'" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlCard'">
                <CrmlCard :title="customLabel || 'TACTILE CONTAINER'" subtitle="ZERO BLUR AGENTS" :variant="activeVariant">
                  <p>Standard surface card container with 3px solid ink stroke.</p>
                </CrmlCard>
              </template>

              <template v-else-if="selectedCompId === 'CrmlNeubrutalCard'">
                <CrmlNeubrutalCard :title="customLabel || 'HEAVY NEUBRUTAL CARD'" :header-color="activeVariant">
                  <p>Heavy drop-shadow container card with header banner.</p>
                </CrmlNeubrutalCard>
              </template>

              <template v-else-if="selectedCompId === 'CrmlGlassCard'">
                <CrmlGlassCard :title="customLabel || 'FROSTED GLASS CONTAINER'" :variant="activeVariant">
                  <p>Solid stroke frosted container without soft ambient blur.</p>
                </CrmlGlassCard>
              </template>

              <template v-else-if="selectedCompId === 'CrmlStickyNote'">
                <CrmlStickyNote :title="customLabel || 'CRITICAL MEMO'" :color="activeVariant === 'pink' ? 'peach' : (activeVariant === 'cyan' ? 'mint' : (activeVariant === 'lime' ? 'lavender' : 'yellow'))">
                  <p>Streetwear sticky note container with top tape strip.</p>
                </CrmlStickyNote>
              </template>

              <template v-else-if="selectedCompId === 'CrmlDoodleContainer'">
                <CrmlDoodleContainer :title="customLabel || 'DOODLE CONTAINER'" :variant="activeVariant">
                  <p>Playful container with corner accent stickers.</p>
                </CrmlDoodleContainer>
              </template>

              <template v-else-if="selectedCompId === 'CrmlFloatingEmojiBar'">
                <CrmlFloatingEmojiBar :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlAlert'">
                <div style="width: 100%;">
                  <CrmlAlert :variant="activeVariant" :title="customLabel || 'SYSTEM WARNING'">
                    High voltage tactile physical switches armed.
                  </CrmlAlert>
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlToast'">
                <CrmlToast :title="customLabel || 'CRML SYSTEM NOTIFICATION'" message="Component loaded into active sandbox canvas." :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlNavbar'">
                <div style="width: 100%;">
                  <CrmlNavbar :brand-text="customLabel || 'CRML - UI'" brand-badge="v5.5" :items="sampleNavItems" :variant="activeVariant === 'yellow' ? 'lime' : activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlSidebar'">
                <div style="width: 260px;">
                  <CrmlSidebar :title="customLabel || 'MECHA SUITE'" user-handle="@CRML_PILOT" :items="sampleSidebarItems" :variant="activeVariant === 'yellow' ? 'lime' : activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlPillTabs'">
                <CrmlPillTabs v-model="sampleTabVal" :items="sampleTabs" :variant="(activeVariant as any)" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlDropdown'">
                <CrmlDropdown v-model="sampleDropdownVal" :options="sampleDropdownOptions" :label="customLabel || 'SELECT AGENT NODE'" :variant="(activeVariant as any)" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlTagInput'">
                <div style="width: 100%; max-width: 380px;">
                  <CrmlTagInput v-model="sampleTags" :label="customLabel || 'KEYWORDS TAGS'" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlEmojiRatingSlider'">
                <CrmlEmojiRatingSlider v-model="sampleRatingVal" :label="customLabel || 'TACTILE RATING SLIDER'" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlVaporwaveVisualizer'">
                <div style="width: 100%;">
                  <CrmlVaporwaveVisualizer :title="customLabel || 'AUDIO SPECTRUM HYPER-VISUALIZER'" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlRadioGroup'">
                <CrmlRadioGroup v-model="sampleRadioVal" :options="sampleRadioOptions" :label="customLabel || 'EXECUTION MODE'" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlPagination'">
                <CrmlPagination v-model:currentPage="samplePageVal" :total-pages="10" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlBreadcrumb'">
                <CrmlBreadcrumb :items="sampleBreadcrumbItems" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlSearchTagFilter'">
                <div style="width: 100%;">
                  <CrmlSearchTagFilter
                    v-model:search="sampleSearchVal"
                    v-model:active-tag="sampleActiveTag"
                    v-model:selected-tags="sampleSelectedTags"
                    :available-tags="sampleAvailableTags"
                    :variant="activeVariant"
                    :size="activeSize"
                    :disabled="activeDisabled"
                    :multi-select="searchTagMultiSelect"
                    :filter-label-text="searchTagFilterLabel"
                    :show-counts="searchTagShowCounts"
                    :show-shortcut="searchTagShowShortcut"
                    :shortcut-text="searchTagShortcutText"
                    :placeholder="customLabel || 'Search 56 primitives, tokens, scripts...'"
                  />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlStepper'">
                <div style="width: 100%;">
                  <CrmlStepper :steps="sampleSteps" :current-step="1" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlRetroDialog'">
                <CrmlBouncyButton :variant="activeVariant" @click="dialogOpen = true">OPEN {{ customLabel || 'RETRO DIALOG' }}</CrmlBouncyButton>
                <CrmlRetroDialog v-model="dialogOpen" :title="customLabel || 'SYSTEM CONFIRMATION'" :variant="activeVariant">
                  <p>Are you sure you want to execute zero ambient blur compilation?</p>
                </CrmlRetroDialog>
              </template>

              <template v-else-if="selectedCompId === 'CrmlFunkyAccordion'">
                <div style="width: 100%;">
                  <CrmlFunkyAccordion :items="sampleAccordionItems" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlHypebeastDropBanner'">
                <div style="width: 100%;">
                  <CrmlHypebeastDropBanner :title="customLabel || 'LIMITED DROP #009'" subtitle="EXCLUSIVE CYBER PACK" badge-text="LIVE NOW" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlTimeline'">
                <div style="width: 100%;">
                  <CrmlTimeline :items="sampleTimelineItems" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlPricingCard'">
                <CrmlPricingCard
                  :title="customLabel || 'CYBER PRO'"
                  price="$49"
                  period="/month"
                  description="Full access to 56 neubrutalist primitives."
                  :features="samplePricingFeatures"
                  button-text="UPGRADE NOW ⚡"
                  :variant="activeVariant"
                />
              </template>

              <template v-else-if="selectedCompId === 'CrmlCommandPalette'">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 0.75rem;">
                  <CrmlBouncyButton :variant="activeVariant" @click="cmdOpen = true">OPEN {{ customLabel || 'COMMAND PALETTE' }} [⌘K]</CrmlBouncyButton>
                  <CrmlCommandPalette v-model="cmdOpen" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlFilterDrawer'">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 0.75rem;">
                  <CrmlBouncyButton :variant="activeVariant" @click="drawerOpen = true">OPEN {{ customLabel || 'FILTER DRAWER' }}</CrmlBouncyButton>
                  <CrmlFilterDrawer v-model="drawerOpen" :variant="activeVariant" :title="customLabel || 'ADVANCED FILTERS'" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlKanbanCard'">
                <CrmlKanbanCard :title="customLabel || 'IMPLEMENT DESIGN SYSTEM'" tag="IN PROGRESS" priority="P0" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlSwipeCard'">
                <CrmlSwipeCard :title="customLabel || 'CYBER MECHA ITEM'" subtitle="SWIPE LEFT OR RIGHT" :variant="activeVariant" />
              </template>

              <template v-else-if="selectedCompId === 'CrmlNotificationDrawer'">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 0.75rem;">
                  <CrmlBouncyButton :variant="activeVariant" @click="notifOpen = true">TOGGLE {{ customLabel || 'NOTIFICATIONS' }}</CrmlBouncyButton>
                  <CrmlNotificationDrawer v-model="notifOpen" :notifications="sampleNotifications" :variant="activeVariant" />
                </div>
              </template>

              <template v-else-if="selectedCompId === 'CrmlReviewQuoteCard'">
                <CrmlReviewQuoteCard author="ALEX_CYBER" role="LEAD ARCHITECT" :quote="customLabel || 'The most tactile Vue 3 component library ever built.'" :rating="5" :variant="activeVariant" />
              </template>

              <template v-else-if="componentMap[selectedCompId]">
                <component :is="componentMap[selectedCompId]" :variant="(activeVariant as any)" :disabled="activeDisabled">
                  {{ customLabel || activeComp.name }}
                </component>
              </template>

              <template v-else>
                <div class="generic-preview-msg">
                  <span style="font-size: 2.5rem;">📦</span>
                  <h4>{{ activeComp.name }}</h4>
                  <p>Interactive preview ready for compilation.</p>
                </div>
              </template>
            </div>
          </div>

          <!-- Interactive Props Controller Box -->
          <div class="props-controls-box">
            <div class="box-bar font-mono">PROPS & VARIANT CONTROLS</div>

            <div class="controls-form">
              <!-- Variant Picker -->
              <div class="control-field">
                <label class="field-label">COLOR VARIANT</label>
                <CrmlSegmentedControl v-model="activeVariant" :options="variantOptions" variant="lime" />
              </div>

              <!-- Size Picker -->
              <div class="control-field" v-if="['CrmlBouncyButton', 'CrmlButton', 'CrmlBlobAvatar'].includes(selectedCompId)">
                <label class="field-label">SIZE TIER</label>
                <CrmlSegmentedControl v-model="activeSize" :options="sizeOptions" variant="lime" />
              </div>

              <!-- Disabled State Toggle -->
              <div class="control-field" v-if="['CrmlBouncyButton', 'CrmlButton', 'CrmlInput', 'CrmlTextarea', 'CrmlNumberInput', 'CrmlCheckbox', 'CrmlNeonSwitch', 'CrmlOtpInput'].includes(selectedCompId)">
                <label class="field-label">DISABLED STATE</label>
                <CrmlNeonSwitch v-model="activeDisabled" label="IS DISABLED" />
              </div>

              <!-- Date Picker Mode Picker -->
              <div class="control-field" v-if="selectedCompId === 'CrmlDatePicker'">
                <label class="field-label">PICKER MODE</label>
                <CrmlSegmentedControl v-model="dateMode" :options="dateModeOptions" variant="pink" />
              </div>

              <!-- Time Picker Format Selector -->
              <div class="control-field" v-if="selectedCompId === 'CrmlTimePicker'">
                <label class="field-label">TIME FORMAT</label>
                <CrmlSegmentedControl v-model="timeFormatVal" :options="timeFormatOptions" variant="cyan" />
              </div>

              <!-- Sticker Tilt Angle Selector -->
              <div class="control-field" v-if="selectedCompId === 'CrmlStickerBadge'">
                <label class="field-label">TILT ANGLE</label>
                <CrmlSegmentedControl v-model="activeTilt" :options="tiltOptions" variant="lime" />
              </div>

              <!-- Sticker Star Accent Toggle -->
              <div class="control-field" v-if="selectedCompId === 'CrmlStickerBadge'">
                <label class="field-label">STAR ACCENT BADGE</label>
                <CrmlNeonSwitch v-model="activeStar" label="SHOW STAR" />
              </div>

              <!-- Wavy Divider Style Selector -->
              <div class="control-field" v-if="selectedCompId === 'CrmlWavyDivider'">
                <label class="field-label">DIVIDER STYLE</label>
                <CrmlSegmentedControl v-model="dividerStyle" :options="dividerOptions" variant="pink" />
              </div>

              <!-- Value / Progress Meter Spinner -->
              <div class="control-field" v-if="['CrmlProgress', 'CrmlSpeedometerDial', 'CrmlFileUpload', 'CrmlPriceTagWidget'].includes(selectedCompId)">
                <label class="field-label">VALUE / PROGRESS (%)</label>
                <CrmlNumberInput v-model="progressVal" :min="0" :max="100" unit="%" />
              </div>

              <!-- Marquee Speed Spinner -->
              <div class="control-field" v-if="selectedCompId === 'CrmlMarquee'">
                <label class="field-label">SPEED (SECONDS)</label>
                <CrmlNumberInput v-model="marqueeSpeed" :min="2" :max="60" unit="SEC" />
              </div>

              <!-- Label Input -->
              <div class="control-field">
                <label class="field-label">CUSTOM TEXT / LABEL</label>
                <CrmlInput v-model="customLabel" placeholder="Type custom label text..." />
              </div>

              <!-- Dual Range Slider Custom Controls -->
              <template v-if="selectedCompId === 'CrmlDualRangeSlider'">
                <div class="control-field">
                  <label class="field-label">COMPACT FORMAT (K / M / B)</label>
                  <CrmlNeonSwitch v-model="rangeCompact" label="COMPACT NOTATION" />
                </div>
                <div class="control-field">
                  <label class="field-label">SHOW BOUND LIMIT CHIPS</label>
                  <CrmlNeonSwitch v-model="showRangeLimits" label="SHOW MIN / MAX CHIPS" />
                </div>
                <div class="control-field">
                  <label class="field-label">VALUE PREFIX</label>
                  <CrmlInput v-model="rangePrefix" placeholder="e.g. $, Rp, €..." />
                </div>
                <div class="control-field">
                  <label class="field-label">VALUE SUFFIX</label>
                  <CrmlInput v-model="rangeSuffix" placeholder="e.g. /mo, IDR..." />
                </div>
              </template>

              <!-- CrmlSearchTagFilter Custom Controls -->
              <template v-if="selectedCompId === 'CrmlSearchTagFilter'">
                <div class="control-field">
                  <label class="field-label">MULTI-SELECT TAGS</label>
                  <CrmlNeonSwitch v-model="searchTagMultiSelect" label="ALLOW MULTI TAGS" />
                </div>
                <div class="control-field">
                  <label class="field-label">SHOW TAG COUNTS</label>
                  <CrmlNeonSwitch v-model="searchTagShowCounts" label="SHOW (COUNT) NUMBERS" />
                </div>
                <div class="control-field">
                  <label class="field-label">SHOW SHORTCUT CHIP</label>
                  <CrmlNeonSwitch v-model="searchTagShowShortcut" label="SHOW SHORTCUT" />
                </div>
                <div class="control-field">
                  <label class="field-label">FILTER ROW LABEL</label>
                  <CrmlInput v-model="searchTagFilterLabel" placeholder="e.g. FILTER:, CATEGORIES:" />
                </div>
                <div class="control-field">
                  <label class="field-label">SHORTCUT CHIP TEXT</label>
                  <CrmlInput v-model="searchTagShortcutText" placeholder="e.g. ⌘K, CTRL+F" />
                </div>
              </template>

              <!-- Button Custom Icon Controls -->
              <template v-if="['CrmlButton', 'CrmlBouncyButton'].includes(selectedCompId)">
                <div class="control-field">
                  <label class="field-label">LEFT ICON (EMPTY TO HIDE)</label>
                  <CrmlInput v-model="btnIconLeft" placeholder="e.g. ⚡, 📁, 🚀 or empty..." />
                </div>
                <div class="control-field">
                  <label class="field-label">RIGHT ICON (EMPTY TO HIDE)</label>
                  <CrmlInput v-model="btnIconRight" placeholder="e.g. ➜, ★, ⚡ or empty..." />
                </div>
              </template>

              <!-- Unit Badge Input (for CrmlNumberInput) -->
              <div class="control-field" v-if="selectedCompId === 'CrmlNumberInput'">
                <label class="field-label">UNIT BADGE TEXT (EMPTY TO HIDE)</label>
                <CrmlInput v-model="unitBadgeText" placeholder="e.g. QTY, PCS, ITEMS or empty..." />
              </div>
            </div>
          </div>
        </div>

        <!-- All Variants Gallery Section -->
        <div class="variants-gallery-card">
          <h3 class="gallery-title font-heading">
            🎨 {{ activeComp.name }} VARIANT GALLERY & MODES
          </h3>

          <div class="variants-grid">
            <div class="variant-item" v-for="v in (['lime', 'pink', 'cyan', 'yellow'] as const)" :key="v">
              <span class="v-label">VARIANT: {{ v.toUpperCase() }}</span>
              <component
                :is="componentMap[selectedCompId]"
                :variant="(v as any)"
                :color="(v as any)"
                :accent-color="(v as any)"
                :header-color="(v as any)"
                :disabled="activeDisabled"
                :size="(activeSize as any)"
                :items="selectedCompId === 'CrmlPillTabs' ? sampleTabs : sampleNavItems"
                ...
                :features="samplePricingFeatures"
                :title="customLabel || activeComp.name"
                :label="customLabel || activeComp.name"
                :text="customLabel || activeComp.name"
                :value="progressVal"
              >
                {{ customLabel || activeComp.name }}
              </component>
        ...
        <!-- Code Snippet Documentation Box -->
        <div class="code-doc-card">
          <h3 class="gallery-title font-heading">
            📋 EXAMPLE CODE & INTEGRATION SNIPPET
          </h3>
          <CrmlCodeBlock :code="generatedCodeSnippet" :title="`${activeComp.name}.vue`" />
        </div>
        ...
        <div class="api-table-card">
          <div class="section-header-row">
            <h3 class="gallery-title font-heading">
              ⚙️ PROPS & PARAMETERS SPECIFICATION
            </h3>
            <span class="spec-count-badge font-mono">{{ (activeComp.apiData || []).length }} PROPS</span>
          </div>
          <p class="section-subtext">
            All component parameters are configurable via Vue props without hardcoded strings or static themes.
          </p>
          <CrmlTable :columns="apiColumns" :data="activeComp.apiData || []" accent-color="lime" />
        </div>

        <!-- Slots Reference Table Card -->
        <div v-if="activeComp.slotsData && activeComp.slotsData.length > 0" class="api-table-card">
          <div class="section-header-row">
            <h3 class="gallery-title font-heading">
              🔌 SLOTS & TEMPLATE MODULARITY
            </h3>
            <span class="spec-count-badge font-mono">{{ activeComp.slotsData.length }} SLOTS</span>
          </div>
          <p class="section-subtext">
            Named and scoped slots allow total customization and decorative overrides while maintaining Neubrutalist physical borders and shadows.
          </p>
          <CrmlTable :columns="slotColumns" :data="activeComp.slotsData" accent-color="cyan" />
        </div>

        <!-- Events Reference Table Card -->
        <div v-if="activeComp.eventsData && activeComp.eventsData.length > 0" class="api-table-card">
          <div class="section-header-row">
            <h3 class="gallery-title font-heading">
              ⚡ EVENTS & EMITS SPECIFICATION
            </h3>
            <span class="spec-count-badge font-mono">{{ activeComp.eventsData.length }} EVENTS</span>
          </div>
          <p class="section-subtext">
            Standard Vue 3 emits triggered on user interactions, bound via v-model or custom event listeners.
          </p>
          <CrmlTable :columns="eventColumns" :data="activeComp.eventsData" accent-color="pink" />
        </div>
      </main>
    </div>
  </div>
...
import {
  ...
  CrmlColorPicker,
  CrmlDualRangeSlider,
  CrmlFloatingEmojiBar,
  CrmlPillTabs,
  CrmlTagInput,
  CrmlDropdown,
  CrmlEmojiRatingSlider,
  CrmlVaporwaveVisualizer,
  CrmlDoodleContainer,
  CrmlRadioGroup,
  CrmlPagination,
  CrmlBreadcrumb,
...
const selectedCompId = ref('CrmlDatePicker')
const activeVariant = ref<'lime' | 'pink' | 'cyan' | 'yellow'>('lime')
const activeSize = ref<'sm' | 'md' | 'lg'>('md')
const customLabel = ref('')
const dateMode = ref<'single' | 'range'>('range')
const unitBadgeText = ref('QTY')
const timeFormatVal = ref<'12h' | '24h'>('12h')

const activeDisabled = ref(false)
const activeStar = ref(true)
const activeTilt = ref<'none' | 'left' | 'right'>('left')
const dividerStyle = ref<'wave' | 'zigzag'>('wave')
const marqueeSpeed = ref(12)
const progressVal = ref(78)

const tiltOptions: SegmentOption[] = [
  { id: 'left', label: 'Tilt Left' },
  { id: 'right', label: 'Tilt Right' },
  { id: 'none', label: 'Flat' }
]

const dividerOptions: SegmentOption[] = [
  { id: 'wave', label: 'Wave' },
  { id: 'zigzag', label: 'Zigzag' }
]

const dateVal = ref('2026-09-12')
const timeVal = ref('09:30 AM')
const numVal = ref(4)
const inputVal = ref('tactile_user')
const textareaVal = ref('Synthesizing 56 primitives.')
const richVal = ref('<b>CRML Storybook Editor</b>')
const colorVal = ref('#CCFF00')
const rangeMin = ref(150)
const rangeMax = ref(800)
const rangePrefix = ref('$')
const rangeSuffix = ref('')
const rangeCompact = ref(false)
const showRangeLimits = ref(true)
const checkboxVal = ref(true)
const switchVal = ref(true)
const segmentedVal = ref('lime')
const otpVal = ref('559021')
...
const sampleDropdownVal = ref('1')
const sampleDropdownOptions = [
  { value: '1', label: 'PRIMARY_NODE' },
  { value: '2', label: 'BACKUP_NODE' },
  { value: '3', label: 'STANDBY_NODE' }
]

const sampleTags = ref(['vue3', 'neubrutalism', 'y2k'])
const sampleRatingVal = ref(4)

const sampleRadioVal = ref('opt1')
const sampleRadioOptions = [
  { value: 'opt1', label: 'ARCADE MODE' },
  { value: 'opt2', label: 'CYBER MODE' }
]

const samplePageVal = ref(1)
...
const sampleSearchVal = ref('')
const sampleActiveTag = ref('neubrutalism')
const sampleSelectedTags = ref(['neubrutalism', 'cyber'])
const sampleAvailableTags = [
  { id: 'neubrutalism', label: 'Neubrutalism', count: 18, color: 'lime' as const },
  { id: 'y2k', label: 'Y2K Cyber', count: 12, color: 'cyan' as const },
  { id: 'streetwear', label: 'Streetwear', count: 8, color: 'pink' as const },
  { id: 'mecha', label: 'Mecha Kit', count: 15, color: 'yellow' as const }
]

const searchTagMultiSelect = ref(false)
const searchTagFilterLabel = ref('FILTER:')
const searchTagShowCounts = ref(true)
const searchTagShowShortcut = ref(true)
const searchTagShortcutText = ref('⌘K')

const btnIconLeft = ref('⚡')
const btnIconRight = ref('')
...
const sampleAccordionItems = [
  { id: '1', title: 'PHYSICS ENGINE', content: 'Uses solid 3px ink stroke with 4px mechanical offset drop-shadow.' },
  { id: '2', title: 'TOKENS', content: 'Built with Electric Lime (#CCFF00) and Hot Pink (#FF007F).' }
]

const sampleTimelineItems = [
  { id: '1', date: '09:00 AM', title: 'INITIALIZE ENGINE', desc: 'Loaded 56 primitives cleanly.' },
  { id: '2', date: '10:30 AM', title: 'VERIFY PHYSICS', desc: 'Zero ambient blur validated.' }
]

const samplePricingFeatures = [
  { text: 'Unlimited Cyber Telemetry', included: true },
  { text: 'Zero Ambient Blur Shadows', included: true },
  { text: '24/7 Arcade Support', included: true }
]

const sampleNotifications = [
  { id: '1', title: 'SYSTEM ALERT', message: 'Voltage spike detected on Node #04.', time: '2m ago' }
]
...
const sampleSidebarItems = [
  ...
  { id: '2', label: 'MECHA_KIT', icon: '🛠️' },
  { id: '3', label: 'SYSTEM_LOGS', icon: '📟' }
]

const codeSnippetSample = `npm install crml-ui
# or
yarn add crml-ui`

const tableColumns: CrmlTableColumn[] = [
...
const tableData = [
  { id: '#001', name: 'CYBER_01', role: 'ARCHITECT', status: 'ACTIVE' },
  { id: '#002', name: 'NEO_PILOT', role: 'ENG', status: 'ACTIVE' },
  { id: '#003', name: 'ZINE_DEV', role: 'DESIGNER', status: 'STANDBY' }
]
...
const dateModeOptions: SegmentOption[] = [
  { id: 'single', label: 'Single Date' },
  { id: 'range', label: 'Range Date ✦' }
]

const timeFormatOptions: SegmentOption[] = [
...
interface ComponentItem {
  ...
  badge?: string
  designNotes?: string
  apiData: Array<{ prop: string; type: string; default: string; desc: string }>
  slotsData?: Array<{ slot: string; bindings: string; desc: string }>
  eventsData?: Array<{ event: string; payload: string; desc: string }>
}

const catalog: Array<{ name: string; items: ComponentItem[] }> = [
  {
...
const filteredCatalog = computed(() => {
  if (!searchQuery.value) return catalog
  const q = searchQuery.value.toLowerCase()
  return catalog.map(cat => ({
    name: cat.name,
    items: cat.items.filter(item => item.name.toLowerCase().includes(q) || item.desc.toLowerCase().includes(q))
  })).filter(cat => cat.items.length > 0)
})

const activeComp = computed(() => {
  for (const cat of catalog) {
    const found = cat.items.find(i => i.id === selectedCompId.value)
    if (found) return found
  }
  return catalog[0].items[0]
})

const apiColumns: CrmlTableColumn[] = [
  { key: 'prop', label: 'PROP', sortable: true, width: '22%' },
  { key: 'type', label: 'TYPE', width: '24%' },
  { key: 'default', label: 'DEFAULT', width: '18%' },
  { key: 'desc', label: 'DESCRIPTION & USAGE' }
]

const slotColumns: CrmlTableColumn[] = [
  { key: 'slot', label: 'SLOT NAME', sortable: true, width: '22%' },
  { key: 'bindings', label: 'SCOPED BINDINGS', width: '26%' },
  { key: 'desc', label: 'DESCRIPTION & USE CASE' }
]

const eventColumns: CrmlTableColumn[] = [
  { key: 'event', label: 'EVENT NAME', sortable: true, width: '22%' },
  { key: 'payload', label: 'PAYLOAD TYPE', width: '26%' },
  { key: 'desc', label: 'TRIGGER CONDITION' }
]

const generatedCodeSnippet = computed(() => {
  if (selectedCompId.value === 'CrmlDatePicker') {
    if (dateMode.value === 'range') {
      return `<script setup lang="ts">
import { ref } from 'vue'
import { CrmlDatePicker } from 'crml-ui'

const dateRange = ref(['2026-09-05', '2026-09-18'])
<\/script>
...
<template>
  <CrmlDatePicker
    ...
    label="${customLabel.value || 'PROJECT SPRINT DATES'}"
  />
</template>`
    }
    return `<script setup lang="ts">
...
<template>
  <CrmlDatePicker
    ...
    mode="single"
    variant="${activeVariant.value}"
    label="${customLabel.value || 'SELECT BIRTHDATE'}"
  />
</template>`
  } else if (selectedCompId.value === 'CrmlNumberInput') {
    return `<script setup lang="ts">
import { ref } from 'vue'
import { CrmlNumberInput } from 'crml-ui'
...
<template>
  <CrmlNumberInput
    v-model="quantity"
    variant="${activeVariant.value}"
    label="${customLabel.value || 'PURCHASE QUANTITY'}"
    unit="${unitBadgeText.value}"
    :min="1"
    :max="99"
  />
</template>`
  } else if (selectedCompId.value === 'CrmlTimePicker') {
    return `<script setup lang="ts">
import { ref } from 'vue'
import { CrmlTimePicker } from 'crml-ui'

const selectedTime = ref('09:30 AM')
<\/script>
...
<template>
  <CrmlTimePicker
    ...
    variant="${activeVariant.value}"
    format="${timeFormatVal.value}"
    label="${customLabel.value || 'APPOINTMENT TIME'}"
  />
</template>`
  } else if (selectedCompId.value === 'CrmlDualRangeSlider') {
    return `<script setup lang="ts">
import { ref } from 'vue'
import { CrmlDualRangeSlider } from 'crml-ui'

const minVal = ref(${rangeMin.value})
const maxVal = ref(${rangeMax.value})
<\/script>
...
<template>
  <CrmlDualRangeSlider
    ...
    v-model:max="maxVal"
    :min-limit="${0}"
    :max-limit="${rangeCompact.value ? 10000000 : 1000}"
    variant="${activeVariant.value}"
    label="${customLabel.value || 'NOMINAL RANGE FILTER'}"
    prefix="${rangePrefix.value}"
    suffix="${rangeSuffix.value}"
    :compact="${rangeCompact.value}"
    :show-limits="${showRangeLimits.value}"
  />
</template>`
  }
  return `<script setup lang="ts">
import { ${activeComp.value.name} } from 'crml-ui'
<\/script>

<template>
  <${activeComp.value.name} variant="${activeVariant.value}">
    ${customLabel.value || activeComp.value.name}
...
.inspector-header-card, .variants-gallery-card, .code-doc-card, .api-table-card {
  background: var(--crml-bg-surface, #ffffff);
  border: 3px solid var(--crt-obsidian, #0d0d0d);
  border-radius: var(--crml-radius-lg, 12px);
  box-shadow: 4px 4px 0px var(--crt-obsidian, #0d0d0d);
...
.design-guidelines-box {
  background-color: var(--crml-bg-elevated, #f4f4f0);
...
.live-preview-box, .props-controls-box {
  background: var(--crml-bg-surface, #ffffff);
  border: 3px solid var(--crt-obsidian, #0d0d0d);
  border-radius: var(--crml-radius-lg, 12px);
  box-shadow: 4px 4px 0px var(--crt-obsidian, #0d0d0d);
...
.canvas-display {
  ...
  background: var(--crml-bg-base, #f4f4f0);
}
