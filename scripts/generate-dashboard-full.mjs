...
console.log('⚡ Generating Fully-Documented Standalone CRML-UI Dashboard...')
...
if (!fs.existsSync(catalogPath)) {
  console.error('catalog.json not found! Run extract-catalog.mjs first.')
  process.exit(1)
}
const catalog = JSON.parse(fs.readFileSync(catalogPath, 'utf8'))

// 2. Write catalog.ts to dashboard src/
const catalogTsContent = `export interface ApiProp {
  prop: string
  type: string
  default: string
  desc: string
...
export const CATALOG_DATA: CatalogCategory[] = ${JSON.stringify(catalog, null, 2)}
`
